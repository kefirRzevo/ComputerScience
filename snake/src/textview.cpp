...
bool TextView::IsInFrame(const Coordinate &point) const {
...
KeyCode TextView::GetPolledKey() const {
...
void TextView::DrawResults() const {
...
void TextView::SetSnakeStyleColors(int snakeStyle) const {
  SetColor(FromInt(9 + snakeStyle), FromInt(1 + snakeStyle));
}

//----------------------------------------//

void TextView::SetRabbitStyleColors(int rabbitStyle) const {
  SetColor(FromInt(17 + 7 * rabbitStyle), FromInt(17 + 9 * rabbitStyle));
}

//----------------------------------------//

void TextView::SetColor(Color fg, Color bg) const {
  if (fg >= 0)
    printf("\e[38;5;%dm", fg);
  if (bg >= 0)
    printf("\e[48;5;%dm", bg);
...
void TextView::HLine(int x0, int y0, int len, int sym) const {
...
void TextView::VLine(int x0, int y0, int len, int sym) const {
...
void TextView::Symbol(Coordinate coord, int sym) const {
...
void TextView::String(int x, int y, const char *string) const {
  printf("\e[%d;%dH", y, x);
  printf("%s", string);
}
...
void TextView::Clear(Color bg) const {
  SetColor(-1, bg);
  printf("\e[2J");
}
...
void TextView::CarretOff() const { printf("\e[?25l"); }

//----------------------------------------//

void TextView::CarretOn() const { printf("\e[?25h"); }

//----------------------------------------//

void TextView::TermiosPropsOn() {
  tcsetattr(STDIN_FILENO, TCSANOW, &(this->termis_attr));
}

//----------------------------------------//

void TextView::TermiosPropsOff() {
