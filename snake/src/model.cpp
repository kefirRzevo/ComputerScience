...
int Model::GetDistance(const Coordinate &coord1, const Coordinate &coord2) {
  return abs(coord1.first - coord2.first) + abs(coord1.second - coord2.second);
}

//----------------------------------------//

void Rabbit::RandomGenerate(Model *model, int score_) {
  assert(model);
...
Snake::Snake(int styleType_)
    : coords({}), newFront({}), dirInFuture(Direction::UP),
      dirAfterUpdate(Direction::UP), styleType(styleType_), scoreReserved(0),
      score(0), alive(true), inGame(true) {
  View *view = View::Get();
  view->SetOnTimer(
      {AFTER_DEATH_MSEC * 1000, std::bind(&Snake::OnTimerDeath, this)});
}

//----------------------------------------//

void Snake::RandomGenerate(Model *model, int snakeSize) {
  int iterator = 0;
  int counter = 0;
...
Coordinate Snake::GetCoordDirection(Snake::Direction dir) {
  switch (dir) {
  case Direction::DOWN:
    return {0, 1};
...
Snake::Direction Snake::GetOppositeDir(Snake::Direction dir) {
  switch (dir) {
...
Model::Model(Size polygonSize_, size_t nSnakes) {
  ...
  View *view = View::Get();
  view->SetOnTimer({TICK_MSEC * 1000, std::bind(&Model::OnTimer, this)});
}
...
void Model::OnTimer() { Update(); }

//----------------------------------------//

Coordinate Model::GetClosestRabbitCoord(const Coordinate &point) const {
  Rabbit *closestRabbit = *rabbits.begin();
  int closestDistance =
      GetDistance(static_cast<Coordinate>(*closestRabbit), point);
...
int Model::GetSnakeIndex(const Snake *snake) const {
  int index = 0;
  for (auto snakeIterator = snakes.begin(); snakeIterator != snakes.end();) {
...
void Snake::Dump() const {
  ...
  fprintf(fp, "NewFront %-2d %-2d; ", GetNewFront().first,
          GetNewFront().second);
...
bool Model::IsThereRabbit(const Coordinate &point) const {
  for (const auto &rabbit : rabbits)
