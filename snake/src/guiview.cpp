...
static sf::IntRect &&CutRect(const sf::Texture &texture, int i, int j,
                             int w = 5, int h = 4);

static bool Y_B_lower(const Coordinate &b, const Coordinate &x);

static sf::Sprite GetSprite(const Coordinate &point, const sf::Texture &texture,
                            const sf::IntRect &textureRect = {});

static bool IsInPolygon(const Size &polygonSize, const Coordinate &point);

static bool Y_B_greater(const Coordinate &b, const Coordinate &x);

static bool X_B_greater(const Coordinate &b, const Coordinate &x);

static bool Y_B_greater(const Coordinate &b, const Coordinate &x);

static bool Y_B_lower(const Coordinate &b, const Coordinate &x);

static Coordinate GetTextureIndx(const Coordinate &a, const Coordinate &b,
                                 const Coordinate &c);
...
static sf::IntRect &&CutRect(const sf::Texture &texture, int i, int j, int w,
                             int h) {
...
static bool X_B_lower(const Coordinate &b, const Coordinate &x) {
  return b.first < x.first;
}
...
GuiView::GuiView() {
  ...
  int height =
      (POLYGON_HEIGHT + PADDING_TOP + PADDING_BOTTOM + 2) * SQUARE_SIZE;
...
GuiView::~GuiView() { sfWindow.close(); }

//----------------------------------------//

void GuiView::DrawRabbits() {
  for (const auto &rabbit : model->rabbits) {
    sf::Sprite rabSprite(GetSprite(rabbit->GetCoordinate(), sfRabbitTexture));
...
void GuiView::DrawSnakes() {
  for (const auto &snake : model->snakes) {
    ...
    for (auto it = coords.begin(); it != coords.end(); it++) {
      ...
      sf::IntRect sectRect =
          CutRect(sfSnakeTexture, textureIndx.first, textureIndx.second);
...
void GuiView::DrawResults() {
  ...
  for (const auto &snake : GetModel()->snakes) {
    ...
    result.move(sumIndent,
                static_cast<float>((PADDING_TOP - 1) * SQUARE_SIZE) / 2);
...
sf::Color GuiView::GetStyleColor(int snakeStyle) const {
  switch (snakeStyle) {
  case 0:
    return sf::Color::Red;
