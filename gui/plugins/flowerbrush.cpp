...
const char *FlowerTool::GetName() const { return "Flower"; }

void FlowerTool::SetHeight(int height) {
  height_ = height;

  char str[kMaxStrLen] = "";
  snprintf(str, kMaxStrLen, "Height: %d", height_);
...
void FlowerTool::SetWidth(int width) {
  ...
  char str[kMaxStrLen] = "";
  snprintf(str, kMaxStrLen, "Width: %d", width_);
  width_label_->SetText(str);
}

FlowerTool::FlowerTool()
    : flower_(nullptr), width_(0), height_(0), panel_(nullptr),
      width_label_(nullptr), height_label_(nullptr), width_slider_(nullptr),
      height_slider_(nullptr), width_slider_callback_(nullptr),
  ...
  height += kTabSize + width_label_->GetHeight();

  panel_->Attach(width_slider_, kTabSize, height);
  height += kTabSize + width_slider_->GetHeight();

  panel_->Attach(height_label_, kTabSize, height);
  height += kTabSize + height_label_->GetHeight();
...
void FlowerTool::ActionBegin(ITexture *canvas, int x, int y) {
  ...
  canvas->CopyTexture(flower_, x - width_ / 2, y - height_ / 2, width_,
                      height_);
}
