...
public:
  MedianFilter(plugin::IAPI *api) : panel_{nullptr} {
      ...
      for (uint32_t y = edge_y; y < height - edge_y; ++y) {
        for (int32_t fx = 0; fx < window_width; ++fx) {
          for (int32_t fy = 0; fy < window_height; ++fy) {
            array[fx + fy * window_width] =
                old_buffer.pixels[x + fx - edge_x + (y + fy - edge_y) * width];
          }
        }
  ...
  virtual const char *GetName() const override { return "Median"; }
...
public:
  Plugin(IAPI *api) : median_filter_{nullptr} {
    median_filter_ = new MedianFilter(api);
  }

  virtual ~Plugin() override { delete median_filter_; }
...
extern "C" IPlugin *Create(IAPI *api) { return new Plugin(api); }
