...
void Filler::Apply(ITexture* canvas) {
    canvas->DrawRect(0, 0, canvas->GetWidth(), canvas->GetHeight(), 0x00'FF'00'FF);    
};
...
void ShapesDrawer::ConstructPreferencePanel() {
    ...
    MakeSliderWithTitleMeasure(150, 15, panel->GetWidth() / 8, panel->GetHeight() / 5 + 120, 0, 255, "green:", panel, green_count, new ChangeColorSlideCallback(this, green_count, COMPONENT_GREEN));
    MakeSliderWithTitleMeasure(150, 15, panel->GetWidth() / 8, panel->GetHeight() / 5 + 180, 0, 255, "blue:",  panel, blue_count,  new ChangeColorSlideCallback(this, blue_count,  COMPONENT_BLUE));
