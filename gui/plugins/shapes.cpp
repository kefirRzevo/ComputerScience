...
enum ShapeType { TYPE_SQUARE, TYPE_CIRCLE };

enum ColorComponent { COMPONENT_RED, COMPONENT_GREEN, COMPONENT_BLUE };

IAPI *api = nullptr;

void MakeSliderWithTitle(int size_x, int size_y, int coord_x, int coord_y,
                         int max_val, int min_val, char *title,
                         IPreferencesPanel *panel, ISliderCallback *callback,
                         int title_size_x = 80, int title_size_y = 30,
                         int ch_size = 25);

void MakeSliderWithTitleMeasure(int size_x, int size_y, int coord_x,
                                int coord_y, int max_val, int min_val,
                                char *title, IPreferencesPanel *panel,
                                ILabel *num_label, ISliderCallback *callback,
                                int title_size_x = 80, int title_size_y = 30,
                                int ch_size = 25);
...
public:
  ChangeColorSlideCallback(ShapesDrawer *drawer, ILabel *num_label,
...
extern "C" void Destroy(plugin::IPlugin *plugin) { delete plugin; };
...
void MakeSliderWithTitle(int size_x, int size_y, int coord_x, int coord_y,
  ...
  ILabel *label = api->GetWidgetFactory()->CreateLabel(
      title_size_x, title_size_y, title, ch_size);
  panel->Attach(label, coord_x + size_x / 2 - title_size_x / 2,
                coord_y - title_size_y);

  ISlider *slider_size =
      api->GetWidgetFactory()->CreateSlider(size_x, size_y, max_val, min_val);
...
void MakeSliderWithTitleMeasure(int size_x, int size_y, int coord_x,
                                ...
                                char *title, IPreferencesPanel *panel,
                                ILabel *num_label, ISliderCallback *callback,
                                int title_size_x, int title_size_y,
                                int ch_size) {
  ...
  panel->Attach(label, coord_x + size_x / 2 - title_size_x / 2,
                coord_y - title_size_y);
  panel->Attach(num_label, coord_x + size_x / 2 + title_size_x / 8,
                coord_y - title_size_y);
...
ChangeShapeeCallback::ChangeShapeeCallback(ShapesDrawer *drawer, ShapeType type)
    : drawer(drawer), type(type) {};
void ChangeShapeeCallback::RespondOnClick() { drawer->SetShape(type); };

ChangeColorSlideCallback::ChangeColorSlideCallback(ShapesDrawer *drawer,
                                                   ILabel *num_label,
                                                   ColorComponent component)
    : drawer(drawer), num_label(num_label), component(component) {};
void ChangeColorSlideCallback::RespondOnSlide(float old_value,
                                              float current_value) {
  ...
  case COMPONENT_RED:
    color &= 0x00'FF'FF'FF;
    color |= static_cast<uint32_t>(current_value) << 24;
    break;
  ...
  color |= 0xFF;
  memset(buf, 0, 4);
  snprintf(buf, 4, "%d", static_cast<int32_t>(current_value));
...
ChangeSizeSlideCallback::ChangeSizeSlideCallback(ShapesDrawer *drawer,
                                                 ILabel *num_label)
    : drawer(drawer), num_label(num_label) {};
void ChangeSizeSlideCallback::RespondOnSlide(float old_value,
                                             float current_value) {
...
void TestCallback::RespondOnClick() { std::cout << "click callback!\n"; };
...
Filler::Filler() { this->ConstructPreferencePanel(); };
...
void ShapesDrawer::ActionBegin(ITexture *canvas, int x, int y) {
  ...
  case TYPE_SQUARE:
    canvas->DrawRect(x - size / 2, y - size / 2, x + size / 2, y + size / 2,
                     color);
    break;
...
void ShapesDrawer::SetSize(int new_size) { size = new_size; };

const char *ShapesDrawer::GetName() const { return "Shapes"; }
...
IPreferencesPanel *ShapesDrawer::GetPreferencesPanel() const { return panel; };
void ShapesDrawer::ConstructPreferencePanel() {
  ...
  ILabel *tool_name =
      api->GetWidgetFactory()->CreateLabel(170, 30, "Shapes drawer", 25);
  panel->Attach(tool_name, panel->GetWidth() / 2 - 170 / 2, 5);

  Point coord();
  int button_x = 5 * panel->GetWidth() / 8;
  int button_y = panel->GetHeight() / 8;
  int dy = 10;
  int button_size_x = 80;
  int button_size_y = 30;

  IButton *circle_button = api->GetWidgetFactory()->CreateButtonWithText(
      button_size_x, button_size_y, "Circle", 25);
  circle_button->SetClickCallback(new ChangeShapeeCallback(this, TYPE_CIRCLE));
  panel->Attach(circle_button, button_x, button_y);

  IButton *square_button = api->GetWidgetFactory()->CreateButtonWithText(
      button_size_x, button_size_y, "Square", 25);
  ...
  MakeSliderWithTitleMeasure(150, 15, panel->GetWidth() / 8,
                             panel->GetHeight() / 5 + 0, 1, 100, "size:", panel,
                             size_count,
                             new ChangeSizeSlideCallback(this, size_count));
  MakeSliderWithTitleMeasure(
      150, 15, panel->GetWidth() / 8, panel->GetHeight() / 5 + 60, 0, 255,
      "red:", panel, red_count,
      new ChangeColorSlideCallback(this, red_count, COMPONENT_RED));
  MakeSliderWithTitleMeasure(
      150, 15, panel->GetWidth() / 8, panel->GetHeight() / 5 + 120, 0, 255,
      "green:", panel, green_count,
      new ChangeColorSlideCallback(this, green_count, COMPONENT_GREEN));
  MakeSliderWithTitleMeasure(
      150, 15, panel->GetWidth() / 8, panel->GetHeight() / 5 + 180, 0, 255,
      "blue:", panel, blue_count,
      new ChangeColorSlideCallback(this, blue_count, COMPONENT_BLUE));

  // ILabel* name_size = api->GetWidgetFactory()->CreateLabel(80, 30, "size",
  // 11); panel->Attach(tool_name, panel->GetWidth() / 8 + 150 / 2 - 40,
  // panel->GetHieght() / 4 - 30); ISlider* slider_size =
  // api->GetWidgetFactory()->CreateSlider(150, 25, 0, 255);
  // slider_size->SetSliderCallback(new TestSliderCallback());
  // panel->Attach(slider_size, panel->GetWidth() / 8, panel->GetHieght() / 4);
};

Plugin::Plugin(Filler *filler, ShapesDrawer *drawer)
    : filler(filler), drawer(drawer) {}
