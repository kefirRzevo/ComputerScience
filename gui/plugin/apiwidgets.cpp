...
IButton *APIWidgetFactory::CreateButtonWithIcon(int width_, int height_,
  ...
  Layout *iconLayout = new Layout{{defIconSize},
                                  Config::defMargin,
                                  Config::defBorder,
                                  {minIconSize},
                                  {maxIconSize}};
...
IButton *APIWidgetFactory::CreateButtonWithText(int width_, int height_,
                                                const char *text_,
                                                int char_size_) {
  APIClickCallback *callback = new APIClickCallback{};
  ...
  Layout *layout = new Layout{{defIconSize},
                              Config::defMargin,
                              Config::defBorder,
                              {minIconSize},
                              {maxIconSize}};
...
ISlider *APIWidgetFactory::CreateDefaultSlider(float min_, float max_) {
  return CreateSlider(Config::defAPI_SliderWidth, Config::defAPI_SliderHeight,
                      static_cast<float>(Config::defAPI_SliderBoxWidth),
                      static_cast<float>(Config::defAPI_SliderBoxHeight), min_,
                      max_);
}

ISlider *APIWidgetFactory::CreateSlider(int width_, int height_, float min_,
                                        float max_) {
  return CreateSlider(
      width_, height_, static_cast<float>(Config::defAPI_SliderBoxWidth),
      static_cast<float>(Config::defAPI_SliderBoxHeight), min_, max_);
}

ISlider *APIWidgetFactory::CreateSlider(int width_, int height_,
                                        float boxWidth_, float boxHeight_,
...
ILabel *APIWidgetFactory::CreateDefaultLabel(const char *text_) {
  return CreateLabel(Config::defAPI_LabelWidth, Config::defAPI_LabelHeight,
                     text_, Config::defTextMaxCapacity);
}

ILabel *APIWidgetFactory::CreateLabel(int width_, int height_,
                                      const char *text_, int char_size_) {
  ...
  Layout *layout =
      new Layout{{def}, Config::defMargin, Config::defBorder, min, max};
...
void APIClickCallback::SetCallback(IClickCallback *callback_) {
  callback = callback_;
}

void APIClickCallback::OnResponse() { callback->RespondOnClick(); }

APISliderCallback::APISliderCallback(ISliderCallback *callback_)
    : callback(callback_) {}

void APISliderCallback::SetCallback(ISliderCallback *callback_) {
  callback = callback_;
}

void APISliderCallback::OnResponse(float value_) {
...
Widget *APIButton::GetBasicWidget() { return button; }

int APIButton::GetWidth() { return button->GetLayout()->GetRectangle().width; }

int APIButton::GetHeight() {
  return button->GetLayout()->GetRectangle().height;
}

void APIButton::SetClickCallback(IClickCallback *callback_) {
...
Widget *APISlider::GetBasicWidget() { return scrollBar; }

int APISlider::GetWidth() {
  return scrollBar->GetLayout()->GetRectangle().width;
}

int APISlider::GetHeight() {
  return scrollBar->GetLayout()->GetRectangle().height;
}

float APISlider::GetValue() { return scrollBar->GetValue(); }

void APISlider::SetValue(float value) { scrollBar->SetValue(value); }

void APISlider::SetSliderCallback(ISliderCallback *callback_) {
  callback->SetCallback(callback_);
}

//----------------------------------------//

APILabel::APILabel(TextIcon *textIcon_) : textIcon(textIcon_) {}

void APILabel::SetText(const char *text_) { textIcon->SetString(text_); }

Widget *APILabel::GetBasicWidget() { return textIcon; }

int APILabel::GetWidth() { return textIcon->GetLayout()->GetRectangle().width; }

int APILabel::GetHeight() {
...
Widget *APIPreferencesPanel::GetBasicWidget() { return panel; }

int APIPreferencesPanel::GetWidth() {
  return panel->GetLayout()->GetRectangle().width;
}

int APIPreferencesPanel::GetHeight() {
  return panel->GetLayout()->GetRectangle().height;
}

void APIPreferencesPanel::Attach(IWidget *widget, int x, int y) {
  // assert(x >=-1 && y >= 0);
