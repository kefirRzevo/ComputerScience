...
public:
  CanvasTitle(TextIcon *titleIcon_, CloseButton *closeButton_,
      ...
      : Widget(new Row{0, 0}, Config::defUpReleaseTexture),
        canvasWidget(canvasWidget_) {
    Attach(titleIcon_);
    Attach(closeButton_);
  }

  static CanvasTitle *GetDefault(const std::string &name_,
                                 CanvasWidget *canvasWidget_) {
...
public:
  Canvas(Layout *layout, Vec2i fullTextureSize_, CanvasWidget *canvasWidget_)
      : Widget(layout, nullptr), canvasWidget(canvasWidget_),
        fullTexture(nullptr), apiFullTexture(nullptr),
    ...
    assert(fullTextureSize.x > rect.width && fullTextureSize.y > rect.height);
  }

  static Canvas *GetDefault(Vec2i fullTextureSize_,
                            CanvasWidget *canvasWidget_) {
  ...
  const RectInt &GetVisiblePart() const { return visiblePart; }

  Vec2i GetFullTextureSize() const { return fullTextureSize; }

  void SetVisibilityBeginX(int value) {
    int range = fullTextureSize.x - visiblePart.width;
    if (value >= 0 && value <= range)
      visiblePart.left = value;
  }

  void SetVisibilityBeginY(int value) {
  ...
  bool ProcessListenerEvent(const Event &event_) override {
    if (event_.type == mouseMoved) {
      Vec2i begin{layout->GetRectangle().left, layout->GetRectangle().top};
...
public:
  HorScrollBarCanvasCallback(Canvas *canvas_) : canvas(canvas_) {}

  void OnResponse(float value) override {
...
public:
  VerScrollBarCanvasCallback(Canvas *canvas_) : canvas(canvas_) {}
...
bool CanvasWidget::ProcessListenerEvent(const Event &event_) {
...
bool CanvasWidget::OnEvent(const Event &event_) {
  if (event_.IsMouseType() && !layout->IsInside(event_.mouse.pos))
    return false;
...
void CanvasWidget::OnLayoutResize() {
