...
void Renderer::Initialize(Window *window_) {
  renderer.reset(new Renderer{window_});
}

Renderer *Renderer::Get() { return renderer.get(); }

void Renderer::SetColor(Color color_) { sfColor = To_SF_Color(color_); }

void Renderer::DrawPixel(Vec2i pos) {
...
void Renderer::DrawPixel(Texture *src, Vec2i pos) {
...
void Renderer::DrawCircle(Vec2i center, int radius) {
...
void Renderer::DrawCircle(Texture *src, Vec2i center, int radius) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::DrawThickLine(Vec2i point, float degreeAngle, float length,
                             int thickness) {
...
void Renderer::DrawThickLine(Texture *src, Vec2i point, float degreeAngle,
...
void Renderer::DrawThickLineSlow(Vec2i p1, Vec2i p2, int thickness) {
...
void Renderer::DrawThickLineSlow(Texture *src, Vec2i p1, Vec2i p2,
                                 int thickness) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
...
void Renderer::DrawLine(Vec2i p1, Vec2i p2) {
  sf::Vertex line[] = {sf::Vertex{sf::Vector2f{static_cast<float>(p1.x),
...
void Renderer::DrawLine(Texture *src, Vec2i p1, Vec2i p2) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::DrawRect(const RectInt &rect) {
...
void Renderer::DrawRect(Texture *src, const RectInt &rect) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::DrawTexture(const Texture *dst) {
  sf::Sprite dstSprite{dst->sfSrcTexture};
  sf::Vector2f newScale{static_cast<float>(sfRenderTarget->getSize().x) /
                            dstSprite.getLocalBounds().width,
                        static_cast<float>(sfRenderTarget->getSize().x) /
                            dstSprite.getLocalBounds().height};

  dstSprite.setScale(newScale);
  sfRenderTarget->draw(dstSprite);
}

void Renderer::DrawTexture(Texture *src, const Texture *dst) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::DrawTexture(const RectInt &srcRect, const Texture *dst) {
  sf::Sprite dstSprite{dst->sfSrcTexture};
...
void Renderer::DrawTexture(Texture *src, const RectInt &srcRect,
                           const Texture *dst) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
...
void Renderer::DrawTexture(const Texture *dst, const RectInt &dstRect) {
  assert(dstRect.width && dstRect.height);
  sf::IntRect dstSfRect{dstRect.left, dstRect.top, dstRect.width,
                        dstRect.height};
...
void Renderer::DrawTexture(Texture *src, const RectInt &srcRect,
                           const Texture *dst, const RectInt &dstRect) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::DrawText(Texture *src, const Text *text) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
bool Renderer::OnRender() const { return sfRenderWindow->isOpen(); }

void Renderer::Clear() { sfRenderTarget->clear(sfColor); }

void Renderer::Clear(Texture *src) {
  sf::RenderTexture sfRenderTexture{};
  sfRenderTexture.create(src->GetWidth(), src->GetHeight());
  sf::Sprite srcSfSprite{src->sfSrcTexture};
  sfRenderTexture.draw(srcSfSprite);
...
void Renderer::Display() { sfRenderWindow->display(); }

void Renderer::Close() { sfRenderWindow->close(); }
