...
bool Event::IsMouseType() const {
  if (type == mousePressed || type == mouseReleased || type == mouseHovered ||
      type == mouseMoved || type == rightButtonPress)
    return true;
...
void EventManager::PushEvent(const Event &event) { events.push(event); }

//----------------------------------------//

bool EventManager::Poll_SF_Event() {
  ...
  case sf::Event::TextEntered: {
    events.push(
        {textEntered, Event::KeyValue{static_cast<int>(sfEvent.text.unicode)}});
    return true;
  }
  ...
  /*
      switch (event.type)
      {
          ...
          case mouseMoved:
          {
              fout << "mouseMoved ";
              fout << event.mouse.pos.x << " " << event.mouse.pos.y << " & " <<
     event.mouse.offset.x << " " << event.mouse.offset.y; break;
          }
          case mouseHovered:
          {
              fout << "mouseHovered ";
