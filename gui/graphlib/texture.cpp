...
std::string SF_RectToString(const sf::IntRect &rect) {
  std::ostringstream string;
  string << rect.left << " " << rect.top << " " << rect.width << " "
         << rect.height;
  return string.str();
}
...
Font::Font(const char *path) { sfFont.loadFromFile(path); }

//----------------------------------------//

FontManager::FontManager() : defaultFont(GetFont(Config::defTextFontPath)) {}
...
Font *FontManager::GetFont(const char *path) {
...
Font *FontManager::GetDefaultFont() { return defaultFont; }
...
std::string Text::GetString() const { return sfText.getString(); }

void Text::SetString(const std::string &string_) { sfText.setString(string_); }

void Text::SetPosition(Vec2i pos_) {
  sfText.setPosition(static_cast<float>(pos_.x), static_cast<float>(pos_.y));
}

void Text::Move(Vec2i delta_) {
  sfText.move(static_cast<float>(delta_.x), static_cast<float>(delta_.y));
}

//----------------------------------------//

Texture::Texture(const char *path_) : managerOwners(false), color(0) {
...
Texture::Texture(Color color_)
    : managerOwners(false),
      sfSrcRect(0, 0, Config::defWindowWidth, Config::defWindowHeight),
      color(color_) {
...
int Texture::GetWidth() const { return sfSrcRect.width; }

int Texture::GetHeight() const { return sfSrcRect.height; }

bool Texture::ManagerOwners() const { return managerOwners; }

Color *Texture::ToBuffer() const {
  sf::Image sfImage = sfSrcTexture.copyToImage();
  unsigned int numberPixels = sfImage.getSize().x * sfImage.getSize().y;

  const Color *oldPixels =
      reinterpret_cast<const Color *>(sfImage.getPixelsPtr());
...
void Texture::FromBuffer(Color *pixels) {
  sfSrcTexture.update(reinterpret_cast<sf::Uint8 *>(pixels));
}

void Texture::FromBuffer(Color *pixels, int w, int h, int x, int y) {
...
Texture *TextureManager::GetTexture(const char *path_) {
  sf::Texture temp{};
  if (!temp.loadFromFile(path_))
    return nullptr;

  sf::IntRect sfRect =
      sf::IntRect{{}, static_cast<sf::Vector2i>(temp.getSize())};
...
Texture *TextureManager::GetTexture(const char *path_, const RectInt &rect_) {
  ...
  sf::IntRect sfRect =
      sf::IntRect{rect_.left, rect_.top, rect_.width, rect_.height};
  std::string key = path_ + SF_RectToString(sfRect);
...
Texture *TextureManager::GetTexture(const char *path_, int i, int j, int w,
                                    int h) {
  if (!(w && h))
    return GetTexture(path_);
  ...
  sf::IntRect sfRect =
      sf::IntRect{width * i / w, height * j / h, width / h, height / h};
  std::string key = path_ + SF_RectToString(sfRect);
...
Texture *TextureManager::GetTexture(Color color_) {
...
Texture *TextureManager::GetDefaultTexture() { return defaultTexture; }
