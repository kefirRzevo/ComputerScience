...
ListOpenerCommand::ListOpenerCommand(DropDownList *list_) : list(list_) {}

void ListOpenerCommand::OnResponse() { list->ChangeVisibility(); }
...
ListOpenerButton::ListOpenerButton(Layout *layout_, DropDownList *list_,
                                   ...
                                   Texture *onPress_)
    : Button(layout_, new ListOpenerCommand{list_}, onRelease_, onHover_,
             onPress_),
      list(list_) {
  list->SetParent(this);
...
ListOpenerButton *ListOpenerButton::GetDefault(const std::string &string_,
                                               DropDownList *list_) {
  Texture *rel = Config::defUpReleaseTexture;
  Texture *hov = Config::defUpHoverTexture;
  Texture *press = Config::defUpPressTexture;

  ListOpenerButton *btn =
      new ListOpenerButton{new Row{}, list_, rel, hov, press};
  btn->Attach(TextIcon::GetDefault(string_));
  return btn;
}

bool ListOpenerButton::GetListVisibility() const {
  return list->GetVisibility();
}

void ListOpenerButton::HideList() { list->Hide(); }

void ListOpenerButton::ShowList() { list->Show(); }

void ListOpenerButton::Attach(Widget *child_) {
  if (child_ == list)
    return;
  else
...
void ListOpenerButton::Detach(Widget *child_) {
  if (child_ == list)
    return;
  else
...
void ListOpenerButton::OnLayoutMove() { list->UpdateDropPoint(); }

void ListOpenerButton::OnLayoutResize() { list->UpdateDropPoint(); }
...
DropDownList *DropDownList::GetDefault() {
  return new DropDownList{Config::defDownReleaseTexture};
}

bool DropDownList::GetVisibility() const { return visible; }

void DropDownList::UpdateDropPoint() {
  if (!parent)
    return;
  ...
  RectInt marginRect = {rect.left - add, rect.top - add, rect.width + 2 * add,
                        rect.height + 2 * add};
...
bool DropDownList::ProcessEvent(const Event &event_) {
  if (visible) {
...
bool DropDownList::ProcessListenerEvent(const Event &event_) {
  if (visible)
    return Widget::ProcessListenerEvent(event_);
  return false;
}

bool DropDownList::OnEvent(const Event &event_) {
  if (visible)
...
void DropDownList::Render() const {
  if (visible)
    Widget::Render();
}

void DropDownList::RenderBorder() const {
  if (visible)
    Widget::RenderBorder();
}

void DropDownList::Hide() { visible = false; }

void DropDownList::Show() { visible = true; }

void DropDownList::ChangeVisibility() { visible ? Hide() : Show(); }
