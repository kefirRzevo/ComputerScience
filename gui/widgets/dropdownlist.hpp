...
public:
  ListOpenerButton(Layout *layout_, DropDownList *list_, Texture *onRelease_,
                   Texture *onHover_ = nullptr, Texture *onPress_ = nullptr);

  static ListOpenerButton *GetDefault(const std::string &string_,
                                      DropDownList *list_);
  bool GetListVisibility() const;

  void HideList();
  void ShowList();

  void Attach(Widget *child_) override;
  void Detach(Widget *child_) override;
...
public:
  ...
  static DropDownList *GetDefault();
  bool GetVisibility() const;

  void UpdateDropPoint();
