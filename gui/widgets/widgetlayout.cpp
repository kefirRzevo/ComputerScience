...
RectInt Layout::GetNewRectangle(Vec2i pos) {
...
BorderPart Layout::GetBorderPart(Vec2i pos_) {
  RectInt borderCol, borderRow, corner;

  borderCol = {rect.left - border, rect.top, border, rect.height};
  ...
  borderRow = {rect.left, rect.top - border, rect.width, border};
  ...
  corner = {rect.left - border, rect.top - border, border, border};
...
Layout::Layout(const RectInt &rect_, int margin_, int border_, Vec2i minSize_,
               Vec2i maxSize_)
...
Layout::Layout(const RectInt &rect_, int margin_, int border_)
    : Layout(rect_, margin_, border_, {rect_.width, rect_.height},
             {rect_.width, rect_.height}) {}

Layout::Layout(Vec2i size, int margin_, int border_)
    : Layout({size}, margin_, border_, size, size) {}

Layout *Layout::GetParent() { return parent; }

Widget *Layout::GetWidget() { return widget; }

const RectInt &Layout::GetRectangle() const { return rect; }

int Layout::GetMargin() const { return margin; }

int Layout::GetBorder() const { return border; }

Vec2i Layout::GetMinSize() const { return minSize; }

Vec2i Layout::GetMaxSize() const { return maxSize; }

void Layout::SetWidget(Widget *widget_) { widget = widget_; }

bool Layout::IsInside(Vec2i pos_) const {
  return rect.IsInside(pos_) || IsInsideBorder(pos_);
}

bool Layout::IsInsideBorder(Vec2i pos_) const {
  if (border == 0)
    return false;
...
void Layout::OnEvent(const Event &event_) {
...
void Layout::Attach(Layout *child_) {
  child_->parent = this;
  child_->OnMove({rect.left, rect.top});
  children.push_back(child_);
}

void Layout::Detach(Layout *child_) {
...
void Layout::OnMove(Vec2i delta_) {
  rect.left += delta_.x;
  rect.top += delta_.y;
...
void Layout::OnResize(const RectInt &rect_) {
  rect = rect_;
...
void Container::OnResize(const RectInt &rect_) {
  rect = rect_;
  PlaceChildren();
  widget->OnLayoutResize();
}
...
Row::Row(int margin_, int border_) : Container(margin_, border_) {}

void Row::Attach(Layout *child_) {
  child_->parent = this;
  children.push_back(child_);
...
void Row::PlaceChildren() {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (sumWidth > children[i]->GetMaxSize().x) {
      sumWidthRemain -=
          children[i]->GetMaxSize().x - children[i]->GetMinSize().x;
  ...
  for (size_t i = 0; i < children.size(); i++) {
    ...
    if (typedChildren[i] == Normal) {
      children[i]->OnResize({sumLeft, rect.top + topIndent,
                             children[i]->GetMinSize().x + resultAdd,
                             rect.height - 2 * topIndent});
    } else {
      children[i]->OnResize({sumLeft, rect.top + topIndent,
                             children[i]->GetMaxSize().x,
                             rect.height - 2 * topIndent});
    }
...
Column::Column(int margin_, int border_) : Container(margin_, border_) {}

void Column::Attach(Layout *child_) {
  child_->parent = this;
  children.push_back(child_);
...
void Column::Detach(Layout *child_) {
  auto found = std::find(children.begin(), children.end(), child_);
  if (found != children.end()) {
    children.erase(found);
    child_->parent = nullptr;
...
void Column::PlaceChildren() {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (sumHeight > children[i]->GetMaxSize().y) {
      sumHeightRemain -=
          children[i]->GetMaxSize().y - children[i]->GetMinSize().y;
      typedChildren[i] = Full;
    } else {
      typedChildren[i] = Normal;
      nNormal++;
  ...
  for (size_t i = 0; i < children.size(); i++) {
    ...
    if (typedChildren[i] == Normal) {
      children[i]->OnResize({rect.left + sideIndent, sumTop,
                             rect.width - 2 * sideIndent,
                             children[i]->GetMinSize().y + resultAdd});
    } else {
      children[i]->OnResize({rect.left + sideIndent, sumTop,
                             rect.width - 2 * sideIndent,
                             children[i]->GetMaxSize().y});
    }
