...
Icon::Icon(Layout *layout_, Texture *texture_) : Widget(layout_, texture_) {}

Icon *Icon::GetDefault(Texture *texture_) {
  Layout *layout = new Layout{{Config::defIconWidth, Config::defIconHeight}};
  return new Icon{layout, texture_};
}

bool Icon::OnEvent(const Event &event_) {
  assert(&event_);
  return false;
}
...
TextIcon *TextIcon::GetDefault(const std::string &string) {
...
bool TextIcon::CheckSize(Vec2i size_) {
  return size_.x < layout->GetRectangle().width &&
         size_.y < layout->GetRectangle().height;
}

void TextIcon::SetString(const char *string) {
  fullString = string;
  if (fullString.size() > textMaxSize)
    fullString.erase(textMaxSize, fullString.size());
...
bool TextIcon::OnEvent(const Event &event_) {
  assert(&event_);
  return false;
}

void TextIcon::Render() const {
  if (texture)
    Widget::Render();
...
void TextLabelResponseTest::OnResponse(const std::string &string_) {
  fprintf(stderr, "%s\n", string_.c_str());
}
...
TextLabel::TextLabel(Layout *layout_, Texture *texture_,
                     ...
                     int textMaxSize_)
    : TextIcon(layout_, textStyle_, texture_, textMaxSize_),
      responce(response_) {}

bool TextLabel::ProcessListenerEvent(const Event &event_) {
...
bool TextLabel::OnEvent(const Event &event_) {
